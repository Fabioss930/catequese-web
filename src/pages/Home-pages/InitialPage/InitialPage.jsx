...
const columns = [
  ...
  { id: 'email', label: 'Email', minWidth: 100,align:'center',align:'center'},
...
function InitialPage(props) {
  const actions = [
    { icon: <SaveIcon />, name: 'Cadastro de Catequizando', },
  ];
...
const handleChangeRowsPerPage = (event) => {
  ...
  return (
    <div>
      <Header title='Paroquia'  />
      <div style={{paddingLeft:50, paddingRight:150}}>
      <div style={{width:200,paddingTop:10,marginBottom:10}}>
        <Form style={{display:'flex', alignItems:'center'}}>
            <Input name='text' placeholder='Pesquisar' style={{marginLeft:20}}></Input>
            <SearchIcon/>
        </Form>
      </div>
                    ...
                    {columns.map((column) => {
                      ...
                      return (
                        <TableCell key={column.id} align={column.align}>
                          {column.id=='action'?(<div><Button variant="contained" sx={[styleButtom,{backgroundColor:'#ff9000'}]}  size='5' onClick={()=>console.log(row.id)} ><Edit fontSize='small'/></Button>
                                                <Button variant="contained" sx={styleButtom} onClick={()=>console.log(row.id)}><Search fontSize='small'/></Button></div> )              
                          :value}
                        </TableCell>
                      );
                    })}
      ...
      />
       ...
       <SpeedDial
        ...
        sx={{ position: 'absolute', bottom: 50, right: 20, height:150, width:150}}
        icon={<SpeedDialIcon />}
      >
        {actions.map((action) => (
          <SpeedDialAction
