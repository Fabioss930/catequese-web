...
const UpdateClasse: React.FC = (props: any) => {
  ...
  const handleChange = (event: SelectChangeEvent<typeof personName>) => {
    ...
    names.forEach((per, id) => {
      //compara de 'names' e 'value', se der meet ele adiciona o id correspondente
      value.forEach((a) => {
        if (a == per.nome) {
          vet.push(per.id);
        }
    ...
    <>
      ...
      <Container>
        <GridBody>
          ...
          <ContentButtons>
            <Button type="button" onClick={handleSubmitForm} style={Cadastrar}>
              Salvar
            </Button>
            <Button style={Cancelar} onClick={(event) => props.navTo(event, 5)}>
              Cancelar
            </Button>
          </ContentButtons>
        </GridBody>
